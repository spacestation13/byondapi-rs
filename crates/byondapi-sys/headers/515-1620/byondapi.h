...
/*
	BYOND public API version 515.1620
 ...
 * @param Pointer to the major version number
 * @param Pointer to the build number
 */
 ...
 * @param Pointer to CByondValue
 * @param Null-terminated UTF-8 string
 ...
 * @param Pointer to CByondValue
 * @param 1-byte teference type
 * @param 4-byte reference ID; for most types, an ID of NONE is invalid
 * @see Byond_TestRef()
 */
 ...
 * Blocks until function is completed.
 * @param Function pointer to u4c function(void*)
 * @param Void pointer (argument to function)
 * @param True if this call should wait for the callback to finish and use the result; false if not
 * @return User-defined 4-byte integer; returns 0 if non-blocking and not on the main thread
 */
 ...
 * Blocks if not on the main thread.
 * @param Object that owns the var
 * @param Var name as null-terminated string
 ...
 * Blocks if not on the main thread. 
 * @param The list
 * @param CByondValue array, allocated by caller (can be null if querying length)
 * @param Pointer to length of array (in items); receives the number of items read on success, or required length of array if not big enough
 ...
 * Blocks if not on the main thread. 
 * @param The list
 * @param CByondValue array
 * @param Number of items to write
 * @return True on success; false with *len=0 for failure; false with *len=required size if array is not big enough
 */
 ...
 * Blocks if not on the main thread. 
 * @param The list
 * @param The index in the list (may be a number, or a non-number if using associative lists)
 * @param Result
 * @return True on success
 */
 ...
 * Blocks if not on the main thread. 
 * @param The object that owns the proc
 * @param Proc name as null-terminated string
 ...
 * Blocks if not on the main thread. 
 * @param The value to convert to text
 * @param char array, allocated by caller (can be null if querying length)
 * @param Pointer to length of array in bytes; receives the string length (including trailing null) on success, or required length of array if not big enough
 ...
 * Blocks if not on the main thread. 
 * @param One corner of the block
 * @param Another corner of the block
 * @param CByondValue array, allocated by caller (can be null if querying length)
 * @param Pointer to length of array (in items); receives the number of items read on success, or required length of array if not big enough
 * @return True on success; false with *len=0 for failure; false with *len=required size if array is not big enough
 */
 ...
 * Blocks if not on the main thread. 
 * @param The value
 * @param Result, as a CByondValue (intended for future possible override of length)
 * @return True on success
 */
 ...
 * Blocks if not on the main thread. 
 * @param The type to locate
 * @param The list to locate in; can be a null pointer instead of a CByondValue to locate(type) without a list
 * @param Result; can be null if nothing is found
 ...
 * Blocks if not on the main thread. 
 * @param The x,y,z coords
 * @param Result; can be null if coords are invalid
 * @return True (always)
 */
 ...
 * Blocks if not on the main thread. 
 * @param The type to create (type path or string)
 * @param Array of arguments
 * @param Number of arguments
 * @param Result
 * @return True on success
 */
DUNGPUB bool Byond_New(CByondValue const *type, CByondValue const *argA, u4c argS, CByondValue *result);

/**
 * Equivalent to calling new type(arglist)
 * Blocks if not on the main thread. 
 * @param The type to create (type path or string)
 * @param Arguments, as a reference to an arglist
 * @param Result
 * @return True on success
 */
 ...
 * Blocks if not on the main thread. 
 * @param The object to read
 * @param ointer to CByondXYZ result
 * @return True on success
 */
 ...
 * Blocks if not on the main thread. 
 * @param Pointer to the reference to test; will be filled with null if the reference is invalid
